...
void ocall_read(int fd, void *buf, size_t len, size_t *blen)
{
    int	rc;
    OCALL_PRINT(oflag);
    rc = read(fd, buf, len);
...
void
ocall_pica_fsize(const char *path, size_t *sz, int *fd)
{
    off_t	off;
...
void
ocall_pica_fread(int fd, uint8_t *buf,  size_t bufsz, size_t *wsz)
{
    size_t	sz;
    sz = read(fd, buf, bufsz);
    if (sz != bufsz) {
	printf("%s: Cannot read whole file (%ld != %ld)\n",
	       __func__, sz, bufsz);
    }
    *wsz = sz;
    close(fd);
